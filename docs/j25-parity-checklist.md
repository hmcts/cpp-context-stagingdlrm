...
> start); this file records the *result* of applying that method to this repo, re-derived from scratch
> on 2026-09-04 against
> [`docs/pipeline/DD-43191-DD-43192-j25-parity/02-design.md`](pipeline/DD-43191-DD-43192-j25-parity/02-design.md)
> — no earlier DD-43192 planning or implementation was reused.
>
...
| BC-11 | **re-scoped** (was: high) | ~~JSON-P provider resolution / classpath state~~ **corrected**: `JsonObjects.createObjectBuilder().add(key, null)` throws `NullPointerException` identically on J17 and J25 — a pre-existing latent-bug parity, not a J25 regression (parity-method ADR decision 8, added 2026-09-04) | `StagingDlrmCommandHelperTest.generateErrorMigratedCaseSubmissionPayloadThrowsNpeParityWhenResponseStringIsNull` — pins the real call site (`StagingDlrmCommandHelper.generateErrorMigratedCaseSubmissionPayload`'s `.add("errorMessage", responseString)`, `responseString` reachable as null on the error path) | 🟢 | `mvn -o test -pl stagingdlrm-azure-functions -Dtest=StagingDlrmCommandHelperTest` → `Tests run: 13, Failures: 0` (2026-09-04) |
...
| BC-08 | thin | Jackson `'Z'` → `ZoneOffset.UTC` — the repo's only `ZonedDateTime` is in an event-processor **test helper** (`ObjectBuilder.buildMetaData`), not product code | Annotated in place (Javadoc naming BC-08 and FR14) rather than a new test — authoring one would assert the fixture, not the product | 📝 | N/A — no new test; existing `StagingDlrmEventProcessorTest` coverage already exercises the annotated helper |

**BC-11 note — the correction, and why it matters.** Re-deriving this story from scratch (not reusing
any earlier DD-43192 attempt) surfaced that BC-11's own definition had moved since `01-requirements.md`
was written. The original hypothesis (a JSON-P `ServiceLoader` provider collision, provable by a
classpath-resource count across 5 modules) was superseded on 2026-08-26 by fleet-wide empirical work
(`docs/pipeline/DD-43191-DD-43192-j25-parity/Parity+Testing+Java17+-_+Java25.pdf`): the real, verified
mechanism is a `JsonObjectBuilder` null-value NPE, identical on both runtimes — recorded as **decision 8**
in the parity-method ADR (amended in both DLRM repos). No `ServiceLoader`/classpath-count test was built
here; per decision 8, that would pin the wrong mechanism. The `javax.json` coordinate inventory this
correction supersedes (glassfish vs Parsson GAVs across `command-handler`, `event-listener`,
`domain-event`, `domain-aggregate`, `azure-functions`) remains true classpath fact but is not, on its
own, evidence of a behavioural difference.

**BC-03 note.** Per both the original investigation report and the fleet-wide guide, BC-03 itself
(Drools recompilation silently flipping allow/deny) is **Refuted** — rules are unchanged and fail-closed.
This story's BC-03 row closes a genuine, pre-existing **coverage gap** (the second rule had never been
tested on any JDK) that happens to share the ticket number; it does not mitigate a live J25 risk.
...
- `pojo-generation-plugin`'s `pojo-generation-schema` execution in `stagingdlrm-domain-event` scans the
  *entire test classpath* (`sourceDirectory: CLASSPATH`), including `common-core-domain` and
  ...
  there would be exactly the "maintenance burden" 01-requirements.md's risk notes warn against, and would
  fail for reasons unrelated to this repo's own code. Left to its large existing incidental coverage
  instead (every generated domain-event type this module's own code references must resolve, or the
...
|---|---|---|---|---|
| BC-14 | ⚪ | Every `META-INF/beans.xml` in this repo declares `bean-discovery-mode="all"` explicitly | Confirmed | The upgrade story's own FR (preserving `bean-discovery-mode="all"`) is what keeps this repo unaffected by CDI 4's discovery-mode default change; there is no context-level code to bind a test to |
...
**BC-11** (see the note above) — the repo-local `docs/analysis/j25-upgrade/j25-behavioural-change-investigation-report.md`
still carries the original, uncorrected provider-collision framing for BC-11 (its linked detail file,
`investigations/BC-11.md`, does not exist in this repo at all — a dead link). The fleet-wide PDF's
2026-08-26 correction is what this story's BC-11 row is built against; the local investigation report
has not been updated to match and should be treated as stale for this one entry.

No other corrections were found for this repo's own executed runs — every other item's behaviour, once
verified fresh against the code on 2026-09-04, matched what the investigation report and the fleet-wide
guide both describe.
...
- **BC-13's and DLRM-01's tables diverge sharply on the identical literals `007`/`01`/`.5`.** At the
  ...
  outcome. Two different libraries, two completely different failure *shapes* for the same input — the
  concrete reason FR7 forbids a shared table.
