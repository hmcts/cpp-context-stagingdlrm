...
- [**ADR-005**](../adrs/005-j25-parity-test-method-and-bc-scope.md) — the parity tests this story must
...
| `stagingdlrm-azure-functions` → Java 25 | Verified GA on Azure Functions to May 2029, Windows included. 6 pom items, listed in ADR-006 decision 4, with a documented fallback to 17 |
| BC-12 carve-out | The fleet's `resteasy-client` → `provided` fix must **not** be applied to the func-app — it is a standalone JAR and needs them bundled. ADR-006 decision 5 |
...
neither DLRM repo has that dependency, so no decouple PR (ADR-006 decision 3).
...
| Function App JDK | **Moves to 25.** Fallback to 17 documented in ADR-006 decision 4 if the plugin or library blocks it |
| `anonymise` module | **OPEN** — must be closed at the stage 2 gate. Default if unresolved: retain and migrate. ADR-006 decision 6 |
...
- **`anonymise` module — unresolved.** `defence`, `resulting` and `results` all removed theirs during
  the J25 upgrade; this repo has one, pcfdlrm does not. ADR-006 decision 6 sets out both readings and
