...
| T5 | **done** | `stagingdlrm-domain-value-schema` gains its first `src/test`: `MigratedCaseSubmissionSchemaContractTest` — 9 accept + 11 reject rows as `@ParameterizedTest`, all 11 FR5 pins, validated through `JsonSchemaValidationMatcher` (everit, ADR §6) against the committed root `migrated/migrated-case-submission.json`. Rejects use `failsValidationWithMessage` with the exact everit message; `mvn test` green (20). **Design notes:** (a) `failsValidationWithMessage` does an exact `getMessage().equals`, so the seven `caseDetails` pins (1–6, 9) pin the generic `allOf` wrapper `#/migratedCase/caseDetails: #: only 1 subschema matches out of 2` — this is exactly the terminal message that lands in the outcome file (matches the event-processor test's pinned description), and each pin still goes red when its own constraint is relaxed; pins 7,8,10,11 pin their specific messages. (b) One valid XHIBIT base fixture, each scenario derived by an in-memory mutation written to a `@TempDir` file (the matcher reads a path, not a string) — adding pin 12 is one `Arguments.of` row, no new fixture (stronger than AC10's "two fixtures"; same trade-off T2 took). Deviations: AC9's no-pin-rows comment omitted (repo rule: comment-free test classes) — the 5 over-reported rows and 17-row CSV trace live in this story (lines 760–779). NFR1: no `src/main` change. **FR5a carried to DD-43081 (Q2).** **Added on request:** 4 defensive `DEFENSIVE …` reject rows pinning `migrated-defendant`'s required set (`documentationLanguage`, `hearingLanguage`, `prosecutorDefendantId`, `offences`) — these are **not** DD-43081 relax targets (no `relax-*` CSV row), so they sit outside the AC9 CSV-traceable set and guard against accidental relaxation; 24 tests total (9 accept + 15 reject). |
| T6 | **done — full IT suite 19/19 green on live stack** | **Live run confirmed F1 was masking a real defect + a wrong assertion (not flaky timing).** Root cause: `SystemMapperService.getCaseIdForPtiURN` only forwards when the URN is unmapped or maps to an EJECTED progression case; a mapped non-EJECTED case returns `caseAlreadyProcessedAndExistsInProgression=true` and is NOT forwarded. F1 unmasked two problems, both fixed: **(1) test isolation** — stubs were reset only in `@BeforeAll`, so `shouldAcceptXhibitCaseWhenMappingExistsInSystemIdMapper`'s `stubGetCaseIdByURN("TVL55117DFXXV", 51cac7fb…)` leaked into the 6 other TVL-URN tests, flipping them from "unmapped→forward" to "mapped→not-forwarded"; changed to `@BeforeEach`/`createCommonMockEndpoints()` per test. **(2) wrong assertion** — `…MappingExists…` asserted forwarding for a mapped case, which the processor never does; corrected per product decision to stub progression `EJECTED` for `51cac7fb` so it takes the re-create-and-forward path (the only mapped path that forwards). Prior AC1/AC2/AC5/F2/AC8 changes all stand. **Also flagged, not changed:** ~20 other fixtures still carry `.wav` fileType (out of T6 scope, not whole-payload-asserted). **Not done:** helper→WholePayloadMatcher swap for CaseSubmissionProcessedIT (non-vacuous already). **Remaining ACs:** AC10 (≤10% wall-clock) and AC11 (canary) are CI-observed, not asserted here. | **F1/AC4 fixed** — `PcfdlrmStub.verifyReceiveCaseFileRequested` now does a real `verify(...)` inside `await().untilAsserted(...)` (was a no-op `until(() -> true)`). **AC1/AC2** — 3 `"LIBRA"` literals + 3 LIBRA fixtures re-labelled XHIBIT (incl. `LIBRA-*-id` refs and a `"This is from Libra"` leftover); `grep -r LIBRA resources/` and `'"LIBRA"'` in java both empty. **AC5** — new `verifyReceiveCaseFileBody` does a whole-payload compare of the captured `receive-migrated-case-file` body in the success journey against `expected/receive-migrated-case-file.json` (generated offline from the real convertor/processor), excluding `submissionId`, `caseDetails.caseId`, `materials[*].caseId`, minted `defendants[*].id`/`offences[*].offenceId`. **F2/AC6** — removed the guaranteed-vacuous `getJsonObject("materials"/"caseMarkers"/"plea"/"verdict")` `null==null` assertions and the line-500 `getJsonObject("2024-08-23")` typo + the vacuous `assertNull(getJsonObject(...))` no-ops. **AC8/F12** — `ReceiveErrorCaseSubmissionIT` now asserts `Optional.isPresent()` + payload whole (was `assertThat(Optional, notNullValue())`). IT module `test-compile` green. **Not done:** `MigratedCaseSubmissionEventHelper`→`WholePayloadMatcher` swap / `CaseSubmissionProcessedIT` whole-payload (helper already asserts all 5 fields — non-vacuous; swap deferred due to dynamic per-run ids + no local run). **Cannot verify here (needs Docker/live stack):** AC7 green journeys, AC10 wall-clock, AC11 canary — CI/sandbox. |
