...
**A decision shared with `cpp-context-prosecution-casefile-dlrm` is mirrored there, not
linked across.** The two DLRM repos are worked independently, by separate developers,
and each repo's pipeline must be self-contained — a cross-repo URL breaks that, because
the other repo may not be checked out, may be ahead, or may not yet carry the file. So
the ADR is authored once and committed to **both** repos under the same filename, carrying a header
line that names its mirror (*"Mirrored in
`cpp-context-prosecution-casefile-dlrm/docs/pipeline/adrs/006-…md`. Amend both, or
neither."*). Amendments go in as a pair of PRs; sections that apply to only one repo say
so inline, so the two files stay byte-identical rather than forking. The ADR number
series is shared across the two repos, so `the upgrade-mechanics ADR` means the same document in both.
...
holding both halves in their head. Mirror that ADR into both repos, per the rule above,
so each dev has it locally. Validate both sides afterwards with the
`api-contract-check` skill.

**Neither dev is blocked on the other's gates.** Cross-repo coupling is the contract and
the deploy order, nothing else. A story here that cannot reach stage 8 until something
merges in the other repo is a fact for stage 2a and the sprint plan — never a reason for
this repo's pipeline to pause mid-stage waiting on another repo's gate.
...
### This branch does not carry the LIBRA-era work

`team/25.104.x` is cut from `main`. The `stagingdlrm-test-support` module and its scenario-test DSL,
the source-system-keyed Function App validators, `docs/analysis/libra-ingestion/` and `tools/` all live
on `team/libra1` and are **not here**. (`docs/architecture/` *is* here — it is service reference, not
LIBRA work.) A stage that plans work against them is planning against code
the branch does not contain — see [the parity-method ADR](./adrs/DD-43191-j25-parity-method.md)
decision 7, which re-scopes the J25 stories against what the branch actually holds.

Anything left on disk under those paths is untracked build output from a LIBRA branch, not source.
