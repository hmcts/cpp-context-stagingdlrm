...
> Method and scope are fixed by [ADR-005](../adrs/005-j25-parity-test-method-and-bc-scope.md); this
...
| Depends on | [ADR-005](../adrs/005-j25-parity-test-method-and-bc-scope.md) accepted before stage 5. No other blocker — can start immediately |
...
- **FR3 — Scope is closed.** The items in play are exactly ADR-005's Bucket A for this repo: **BC-03,
  BC-07, BC-08, BC-11, BC-12, BC-13, BC-20, BC-21, DLRM-01**. Adding an item requires ADR-005 amended,
...
- **FR6 — DLRM-01: pin parse behaviour at the Function App gate.** The gate's schema library
  ...
  gate's validators are source-system-keyed ([ADR-002](../adrs/002-source-system-keyed-dispatch.md)),
...
- **FR11 — BC-12: pin the Function App's RESTEasy packaging expectation.** The func-app carries four
  ...
  `NoClassDefFoundError` in Azure. See [ADR-006](../adrs/006-j25-branch-milestone-and-funcapp-jdk.md)
  decision 5.
...
- **FR17 — `docs/j25-parity-checklist.md` is a deliverable, not a by-product.** One row per Bucket A
  and Bucket B item, carrying ADR-005's legend (🟡 authored-not-executed · 🟢 executed green on J17 ·
...
- **The `centos8-j17` agent is a dependency of this story, not an incidental.** If the platform retires
  ...
  ordering in ADR-006 decision 1 has to be revisited.
...
4. **Confirm ADR-002's source-system keying is reachable from a test.** FR6 requires both source
   systems. If `validatorsBySourceSystem` is not injectable, that shapes the test design.
...
6. **`SchemaMatchers` already exists and is shared.** Extending it affects DD-43078/DD-43081's suites.
   Check [ADR-001](../adrs/001-dlrm-scenario-test-dsl.md) before changing its API rather than adding to
   it.
