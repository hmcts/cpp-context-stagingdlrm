...
TRANSFORMATION_JAR=`echo target/stagingdlrm-domain-transformation-anonymise*.jar`
...
[[ ! -f ${EVENT_TOOL_JAR} ]] && \
    curl -k "${MAVEN_ARTIFACT_BASE_URL}/uk/gov/justice/event-tool/${EVENT_TOOL_VERSION}/event-tool-${EVENT_TOOL_VERSION}-swarm.jar" > "${EVENT_TOOL_JAR}"
